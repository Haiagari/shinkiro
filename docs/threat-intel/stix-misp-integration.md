...
- **Geographic & Network Attribution:** Optional MaxMind GeoLite2 enrichment (`internal/intel/geoip`) when `SHINKIRO_GEOLITE2_PATH` / `--geoip-db` points at a `.mmdb` — **no-op when unset**; never invents coordinates. See [`geolite2-geoip.md`](geolite2-geoip.md).
