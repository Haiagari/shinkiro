...
características objetivo:
  ...
  INTERFACES
  - CLI principal (`backend/main.py`) para uso directo
  ...
  - Configuración centralizada en `config/config.yaml` con override por target

BASE DE CÓDIGO ACTUAL
──────────────────────
El framework tiene una versión 1.0 funcional con:
- backend/main.py (orquestador CLI)
- backend/modules/recon.py, ports.py, crawler.py, vuln.py, report.py, utils.py
- `config/config.yaml`, Makefile, requirements.txt
...
patrones de detección. Todo en `config/config.yaml` con defaults sensatos en código.
...
- El `config/config.yaml` con keys sensibles va en `.gitignore` desde el inicio.
