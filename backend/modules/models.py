...
class AgentMemory(Base):
    ...
    mode       = Column(String(50))           # qué modo lo generó (hunt, continuo, etc)
    ...
    expires_at = Column(DateTime, nullable=True) # memoria con TTL (opcional)

class AgentLock(Base):
    __tablename__ = "agent_locks"
    mode       = Column(String(50), primary_key=True)
    locked_at  = Column(DateTime, default=datetime.utcnow)
    expires_at = Column(DateTime)  # auto-release si el proceso murió
