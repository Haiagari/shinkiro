...
from src.storage.models import Subdomain, Port, Vulnerability
...
class PathAnalyzer:
    ...
    def analyze_target_paths(self, target_domain: str) -> List[Dict[str, Any]]:
        ...
        vulnerable_assets = self.db_session.query(Vulnerability).filter(
            Vulnerability.host.like(f"%{target_domain}")
        ).all()
        ...
        for host in vuln_hosts:
            ...
            siblings = self.db_session.query(Subdomain).filter(
                Subdomain.ip == asset.ip,
            ...
            if siblings:
                path = {
                    ...
                    "risk_score": 80 if any(v.severity == 'critical' for v in vulnerable_assets) else 50
                }
                paths.append(path)
