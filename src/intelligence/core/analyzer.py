...
CVSS_WEIGHTS = {"critical": 9.0, "high": 7.0, "medium": 4.0, "low": 1.0, "info": 0.0}
...
class SeverityAnalyzer:
    ...
    def analyze(self, finding: Dict[str, Any]) -> SeverityResult:
        ...
        # Por defecto
        return SeverityResult(
            severity="medium", cvss=4.0, confidence=0.5, reasoning="Default severity"
        )
...
class Deduplicator:
    ...
    def is_known(self, target: str, fingerprint: str) -> bool:
        ...
        return (
            ...
            .filter(
                Finding.target == normalize_lookup_target(target),
                Finding.evidence
                == fingerprint,  # Usamos evidence o una columna dedicada para el hash
            )
            .first()
            is not None
        )
...
class NoveltyDetector:
    ...
    def analyze_novelty(
        ...
        known_findings = (
            self.db.query(Finding.evidence)
            .filter(Finding.target == normalize_lookup_target(target))
            .all()
        )
