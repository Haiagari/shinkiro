...
def run_recon(target: str, out_dir: Path, args) -> dict:
    ...
    if available["httpx"] and resolved_subs:
        ...
        # --- DETECCIÓN DE BLOQUEO WAF/OPSEC ---
        if not live_hosts and resolved_subs:
            log("🛑 ALERTA OPSEC: 0 hosts vivos detectados en subdominios resueltos.", "critical")
            log("Es muy probable un bloqueo de WAF (Cloudflare/Akamai).", "warn")
            log("Sugerencia: Activar Modo Sigilo Extremo o rotar Proxies.", "info")
    else:
