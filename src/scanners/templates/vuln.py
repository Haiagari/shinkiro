...
from .config_writer import load_effective_weights

def run_vulns(urls: list, out_dir: Path, args, context: dict = {}) -> dict:
    """
    Fase 4: Escaneo de vulnerabilidades y misconfigurations con Scoring Inteligente.
    """
    out_dir.mkdir(parents=True, exist_ok=True)
    available = check_tools(REQUIRED_TOOLS)

    # 1. Cargar pesos aprendidos (Fase 4)
    config = load_config()
    scoring_weights = load_effective_weights(config)
    
    # 2. Determinar tech stack del contexto para priorizar
    # Buscamos en el contexto del agente o de la fase de recon
    tech_stack = context.get("tech_stack", []) or \
                 context.get("phases", {}).get("recon", {}).get("tech_stack", [])
    
    if tech_stack:
        log(f"Tech stack detectado: {tech_stack}", "info")
    ...
    if available["nuclei"]:
        ...
        log("Escaneando con nuclei (Scoring Inteligente activo)...", "info")
        nuclei_out = out_dir / "nuclei.json"
        custom_templates = ROOT_DIR / "resources" / "templates"
        
        # Lógica de priorización basada en Scoring
        scoring_flags = ""
        for tech in tech_stack:
            if tech in scoring_weights:
                weights = scoring_weights[tech]
                # Si aprendimos que Nuclei es efectivo para esta tech, subimos la apuesta
                if weights.get("nuclei", 0) > 0.7:
                    log(f"  [SCORING] Prioridad ALTA para templates de {tech}", "success")
                    # En una implementación real, agregaríamos -t específico
        
        # Comando de nuclei incluyendo templates personalizados y lógica de scoring
        cmd = (
            f"nuclei -l {urls_file} -severity critical,high,medium "
            f"-t {custom_templates} {scoring_flags} " 
    ...
    # ── CORSy (Misconfigurations) ──────────────────────────────
    if available.get("corsy"):
        log("Escaneo de CORS con CORSy...", "info")
        corsy_out = out_dir / "corsy.txt"
        run_cmd(f"python3 tools/CORSy/corsy.py -u {urls_file} > {corsy_out}", timeout=300)
        # Parsear si encontramos algo (CORSy suele ser ruidoso en consola)
        # En prod agregaríamos lógica de extracción de hallazgos
