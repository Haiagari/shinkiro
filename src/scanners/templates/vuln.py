...
from src.core.target_normalizer import first_token
...
def run_vulns(urls: list, out_dir: Path, args, context: dict = {}) -> dict:
    ...
    clean_urls = dedupe([first_token(url) for url in urls if first_token(url).startswith("http")])
    if not clean_urls:
    ...
    template_findings = tool_manager.run_capability(
        "template_scan", str(urls_file), rate_limit=50, update=True
    )
    ...
    results = {"findings": findings, "total_findings": len(findings), "out_dir": str(out_dir)}

    save_json(out_dir / "vuln_results.json", results)
    return results
