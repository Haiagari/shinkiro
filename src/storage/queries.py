...
class DBQueries:
    ...
    def get_target(self, domain: str) -> Optional[Target]:
        ...
        return (
            self.db.query(Target).filter(Target.domain == normalize_lookup_target(domain)).first()
        )
    ...
    def get_live_subdomains(self, domain: str) -> List[Subdomain]:
        ...
        return (
            ...
            .join(Scan)
            .filter(and_(Scan.target_id == target.id, Subdomain.is_live == 1))
            .all()
        )
    ...
    def get_open_ports(self, domain: str) -> List[Port]:
        ...
        return (
            self.db.query(Port)
            .join(Scan)
            .filter(and_(Scan.target_id == target.id, Port.state == "open"))
            .distinct(Port.port)
            .all()
        )
    ...
    def get_findings_by_severity(self, domain: str, severity: str) -> List[Vulnerability]:
        ...
        return (
            ...
            .join(Scan)
            .filter(and_(Scan.target_id == target.id, Vulnerability.severity == severity))
            .all()
        )
    ...
    def get_all_findings(self, domain: str) -> List[Vulnerability]:
        ...
        findings = self.db.query(Vulnerability).join(Scan).filter(Scan.target_id == target.id).all()
    ...
    def get_session_history(self, domain: str, limit: int = 20) -> List[ScanSession]:
        ...
        return (
            ...
            .filter(ScanSession.target == normalize_lookup_target(domain))
    ...
    def create_session(self, session_id: str, target: str, mode: str) -> ScanSession:
        ...
        session = ScanSession(session_id=session_id, target=target, mode=mode, status="running")
        self.db.add(session)
        self.db.commit()
        return session
    ...
    def get_session_trace(self, session_id: str) -> Dict[str, Any]:
        ...
        session_row = (
            self.db.query(ScanSession).filter(ScanSession.session_id == session_id).first()
        )
        ...
        if scan:
            ...
            if hypothesis_ids:
                steps = (
                    ...
                    .filter(WorkflowStep.hypothesis_id.in_(hypothesis_ids))
                    .order_by(WorkflowStep.timestamp.asc())
                    .all()
                )
                workflow_steps = [
                    {
                ...
                evidences = (
                    ...
                    .filter(Evidence.hypothesis_id.in_(hypothesis_ids))
        ...
        if scan:
            scan_summary = {
                ...
                if scan.target
                else (session_row.target if session_row else ""),
                "mode": scan.mode,
                "status": scan.status,
                ...
                "errors": [line.strip() for line in scan.errors.splitlines() if line.strip()]
                if scan.errors
                else [],
            }
        ...
        return {
            "session_id": session_id,
            "target": session_summary["target"]
            if session_summary
            else scan_summary["target"]
            if scan_summary
            else "",
            "mode": session_summary["mode"]
            if session_summary
            else scan_summary["mode"]
            if scan_summary
            else "",
            "scan": scan_summary,
    ...
    def get_agent_memory(self, target: str, key: str) -> Optional[AgentMemory]:
        ...
        return (
            ...
            .filter(
                and_(AgentMemory.target == normalize_lookup_target(target), AgentMemory.key == key)
            )
            .first()
        )

    def set_agent_memory(
        self, target: str, key: str, value: Any, mode: str = "hunt", confidence: float = 1.0
    ):
        ...
        if memory:
            ...
            memory = AgentMemory(
                target=target, key=key, value=value, mode=mode, confidence=confidence
            )
            self.db.add(memory)
    ...
    def add_finding_intelligent(
        self, target: str, session_id: str, vuln_data: Dict[str, Any]
    ) -> Finding:
        """
        ...
        existing = (
            ...
            .filter(
                Finding.target == normalize_lookup_target(target), Finding.evidence == fingerprint
            )
            .first()
        )
